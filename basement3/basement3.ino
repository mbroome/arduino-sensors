...
#define MY_NODE_ID BASEMENT3_NODE_ID
...
MyMessage msgTemperature(BASEMENT3_TEMP1, V_TEMP);
...
void presentation() {
  ...
  present(BASEMENT3_TEMP1, S_TEMP);
}
...
void loop()
{
  ...
  if (currentMillis - previousMillis >= SLEEP_TIME) {
    ...
    if (lastTemperature != temperature && temperature > -127.00 && temperature < 185.00) {
      ...
      send(msgTemperature.setSensor(BASEMENT3_TEMP1).set(temperature, 2));
